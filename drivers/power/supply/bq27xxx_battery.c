				      ...
				      struct bq27xxx_dm_reg *reg)
{
	...
	&&  buf->block == reg->offset / BQ27XXX_DM_SZ)
		return (u16*) (buf->block_data + reg->offset % BQ27XXX_DM_SZ);
...
void bq27xxx_battery_settings(struct bq27xxx_device_info *di)
{
	...
	/* if functions don't exist for writing data then abort */
	if (!di->bus.write || !di->bus.write_bulk)
		return;

	/* if no settings to be set for this chipset then abort */
	if (!di->dm_regs)
		return;
